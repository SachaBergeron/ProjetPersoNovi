 ...
 */
final class Version20190415151311 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('ALTER TABLE etudiant ADD COLUMN jamais_co BOOLEAN DEFAULT NULL');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('DROP TABLE __temp__controle');
        $this->addSql('CREATE INDEX IDX_E39396EAFC2B591 ON controle (module_id)');
        $this->addSql('DROP INDEX UNIQ_717E22E3F85E0677');
        $this->addSql('CREATE TEMPORARY TABLE __temp__etudiant AS SELECT id, username, roles, password, nom, prenom, email FROM etudiant');
        $this->addSql('DROP TABLE etudiant');
        $this->addSql('CREATE TABLE etudiant (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username VARCHAR(180) NOT NULL, roles CLOB NOT NULL --(DC2Type:json)
        , password VARCHAR(255) NOT NULL, nom VARCHAR(255) NOT NULL, prenom VARCHAR(255) NOT NULL, email VARCHAR(255) NOT NULL)');
        $this->addSql('INSERT INTO etudiant (id, username, roles, password, nom, prenom, email) SELECT id, username, roles, password, nom, prenom, email FROM __temp__etudiant');
        $this->addSql('DROP TABLE __temp__etudiant');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_717E22E3F85E0677 ON etudiant (username)');
        $this->addSql('DROP INDEX IDX_C242628E455FCC0');
        $this->addSql('CREATE TEMPORARY TABLE __temp__module AS SELECT id, enseignant_id, nom, vol_horaire, coefficient FROM module');
        $this->addSql('DROP TABLE module');
